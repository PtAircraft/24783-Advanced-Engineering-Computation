    ...
    void Rebalance(Node *nodePtr)
    {
        while (nodePtr != nullptr) {
            ...
            if (nodePtr->left != nullptr) {
               heightL = GetHeight(MakeHandle(nodePtr->left));
            } else {
                heightL = 0;
            }
            if (nodePtr->right != nullptr) {
                 heightR = GetHeight(MakeHandle(nodePtr->right));
            }else {
                heightR = 0;
            }
            balance = heightL - heightR;
            //====================================
            // Off balanced node is left heavy
            if(balance >= 2) {
                int heightLL, heightLR;
                if (nodePtr->left->left != nullptr) {
                    heightLL = GetHeight(MakeHandle(nodePtr->left->left));
                } else {
                    heightLL = 0;
                }
                if (nodePtr->left->right != nullptr) {
                    heightLR = GetHeight(MakeHandle(nodePtr->left->right));
                }else {
                    heightLR = 0;
                }
                int balanceL = heightLL - heightLR;
                // Left of the off balanced node is left heavy
                //Left Left case
                if (balanceL >= 0) {
                    RotateRight(MakeHandle(nodePtr));
                }
                // Left of the off balanced node is right heave
                // Left Right case
                if (balanceL < 0) {
                    auto leftnode = nodePtr->left;
                    RotateLeft(MakeHandle(leftnode));
                    leftnode = leftnode->up;
                    RotateRight(MakeHandle(nodePtr));
                }
            }
            //==================================
            // Off balanced node is right heavy
            if(balance <= -2) {
                int heightRL;
                int heightRR;
                if (nodePtr->right->left != nullptr) {
                    heightRL = GetHeight(MakeHandle(nodePtr->right->left));
                } else {
                    heightRL = 0;
                }
                if (nodePtr->right->right != nullptr) {
                    heightRR = GetHeight(MakeHandle(nodePtr->right->right));
                }else {
                    heightRR = 0;
                }
                int balanceR = heightRL - heightRR;
                // Right of the off balanced node is right heavy
                //Right Right case
                if (balanceR <= 0) {
                    RotateLeft(MakeHandle(nodePtr));
                }
                // Right of the off balanced node is left heave
                // Right Left case
                if (balanceR > 0) {
...
//Implement TreeToVine
void treeToVine()
{
    auto rNode = root;
    while(rNode != nullptr) {
        while(rNode->left != nullptr) {
            RotateRight(MakeHandle(rNode));
            rNode = rNode->up;
        }
        rNode = rNode->right;
    }
...
//Implement Compress;
void Compress(int lc) {
    int flag = 0;
    auto cNode = root;
    int num = 0;
    if (cNode->right != nullptr) {
        while(true) {
            RotateLeft(MakeHandle(cNode));
            cNode = cNode->up;
            if(cNode->right != nullptr) {
                cNode = cNode->right;
            } else {
...
//Implement Vine To Tree;
void vineToTree() {
    int sz = GetN();
    int lc = sz + 1 - pow(2,int(log2(sz+1)));
    Compress(lc);
    sz = sz - lc;
    while (1<sz) {
        Compress(sz/2);
