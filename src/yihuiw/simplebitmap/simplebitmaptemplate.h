...
public:
	...
	*/
	void CutOut(SimpleBitmapTemplate <ComponentType,NumComponentPerPixel> &destination,int thisX0,int thisY0,int wid,int hei,ComponentType clearColor) const
	{
      ...
      //  printf("des:%d\n", GetTotalNumComponent(wid,hei));
      //  printf("bmp:%d\n", GetTotalNumComponent(this->nx,this->ny)); //bmp:26624000
		int num = GetNumComponentPerLine();
//        printf("num::%d\n", num);
		int k = 0;
//        printf("test01\n");
        ...
        {
           // printf("test02\n");
            for (int i = thisX0*NumComponentPerPixel; i < (thisX0 + wid)*NumComponentPerPixel; ++i)
            {
             //   printf("test03\n");
                if (i >= num || j >= GetHeight())
                {
                   // printf("test04\n");
                    destination.bmpPtr[k] = clearColor;
                    k++;
                }
                else
                {
                  ...
                  //  printf("k:%d\n", k);
                  //  printf("i + j * num:%d\n", (i + j * num));
                    destination.bmpPtr[k] = this->bmpPtr[i + j * num];
                  //  printf("test06\n");
