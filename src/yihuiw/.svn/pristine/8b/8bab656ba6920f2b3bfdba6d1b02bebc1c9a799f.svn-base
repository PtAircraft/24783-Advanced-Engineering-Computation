...
void HWRenderer::CacheAttributeAndUniformIdent(void)
{
	s = glGetAttribLocation(programIdent, "s");
	t = glGetAttribLocation(programIdent, "t");

	uniformProjectionPos = glGetUniformLocation(programIdent, "projection");
	uniformModelViewPos = glGetUniformLocation(programIdent, "modelView");
	uniformctrlPtsPos = glGetUniformLocation(programIdent, "ctp");
}
