...
void MainWindow::setupTab()
{
    ...
    ui->splitter_2->setStretchFactor(2,1);
...
void MainWindow::on_actionExit_Full_Screen_triggered()
{
    //exit full screen
...
{
    if (oldkey != ui->lineEdit->text()) {
        oldkey = ui->lineEdit->text();
        codeEditor->moveCursor(QTextCursor::Start,QTextCursor::MoveAnchor);
    }

   if(!codeEditor->find(ui->lineEdit->text()/*,QTextDocument::FindCaseSensitively*/)) {
       codeEditor->moveCursor(QTextCursor::Start,QTextCursor::MoveAnchor);
       if(!codeEditor->find(ui->lineEdit->text()/*,QTextDocument::FindCaseSensitively*/)) {
...
{
    search();
//    qDebug() << codeEditor->toPlainText();
    const QString newText = "  sss\n   ";

    codeEditor->setPlainText(newText);
}
