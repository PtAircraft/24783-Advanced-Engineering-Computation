...
public:
	...
	*/
	...
	{
		// It is a very conservative implementation.  Can be made faster.
		destination.Create(wid,hei);
		for(int y=0; y<hei; ++y)
		{
			for(int x=0; x<wid; ++x)
			{
				const int thisX=thisX0+x;
				const int thisY=thisY0+y;
				ComponentType *dstPix=destination.GetEditablePixelPointer(x,y);
				if(this->IsInRange(thisX,thisY))
				{
					const ComponentType *srcPix=GetPixelPointer(thisX,thisY);
