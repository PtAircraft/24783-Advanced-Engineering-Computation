...
/* virtual */ void FsLazyWindowApplication::BeforeEverything(int argc,char *argv[])
{
	...
	if(2<=argc)
	{
		...
		viewTarget=(bbx[0]+bbx[1])/2.0;
		viewDistance=(bbx[1]-bbx[0]).GetLength();
	}
	// Cut & Pasted from stl viewer <<
}
...
/* virtual */ void FsLazyWindowApplication::Draw(void)
{
	...
	drawEnv.SetViewDistance(viewDistance);
...
void FsLazyWindowApplication::RenderShadowBuffer(void) const
{
	...
	drawEnv.SetOrthogonalProjectionHeight(viewDistance);
	drawEnv.SetNearFar(viewDistance/2.0,viewDistance*2.0);
