...
{
private:
	SimpleBitmapTemplate(const SimpleBitmapTemplate<ComponentType,NumComponentPerPixel> &);
	SimpleBitmapTemplate<ComponentType,NumComponentPerPixel> &operator=(const SimpleBitmapTemplate <ComponentType,NumComponentPerPixel> &);
...
public:
	...
	*/
	   ...
	   const SimpleBitmapTemplate <ComponentType,NumComponentPerPixel> &incoming)
	{
		...
		for(int i=0; i<GetTotalNumComponent(incoming.GetWidth(),incoming.GetHeight()); ++i)
		{
			this->bmpPtr[i]=incoming.GetBitmapPointer()[i];
		}
	...
	{
		CleanUp();
		this->nx=incoming.GetWidth();
		this->ny=incoming.GetHeight();
		this->bmpPtr=incoming.GetEditableBitmapPointer();
	...
	const ComponentType *GetPixelPointer(int x,int y) const
	{
		if(IsInRange(x,y))
		{
			return GetBitmapPointer()+(x+y*nx)*NumComponentPerPixel;
		}
		return nullptr;
	}
	...
	ComponentType *GetEditablePixelPointer(int x,int y)
	{
		if(IsInRange(x,y))
		{
			return GetEditableBitmapPointer()+(x+y*nx)*NumComponentPerPixel;
		}
		return nullptr;
	}
